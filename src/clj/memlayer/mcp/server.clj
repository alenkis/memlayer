...
(defn- validate-api-key!
  "Validate MEMLAYER_API_KEY env var against the token store.
   Returns the owner user-id or exits with error."
  [db]
  (let [api-key (System/getenv "MEMLAYER_API_KEY")]
    (when-not api-key
      (binding [*out* *err*]
        (println "MEMLAYER_API_KEY environment variable is required"))
      (System/exit 1))
    (let [key-hash (tokens/sha256 api-key)
      ...
      (cond
        (nil? token)
        (do (binding [*out* *err*]
              (println "Invalid API key: not found in token store"))
            (System/exit 1))

        (:token/revoked-at token)
        (do (binding [*out* *err*]
              (println "Invalid API key: token has been revoked"))
            (System/exit 1))

        :else
        (do (log/info "MCP authenticated as user:" (:token/owner token))
            (:token/owner token))))))
