...
(defn memory-stats-handler
  ...
  (fn [request]
    (let [namespace    (get-in request [:query-params "namespace"])
          ...
          limit        (:memory-limit tuning)
          namespaces   (dh/get-distinct-namespaces db)
          dataset      (when stratum @stratum)
          by-layer     (if namespace
                         (dh/count-by-layer db :namespace namespace)
                         (when dataset
                           (into {}
                                 (map (fn [row] [(:layer row) (:count row)]))
                                 (strat/count-by-layer dataset))))]
      {:status 200
       :body   (cond-> {:namespace-total ns-total
...
(defmethod ig/init-key :handler/stats [_ {:keys [deps]}]
  {:memory-stats (memory-stats-handler (select-keys deps [:db :stratum :tuning]))
