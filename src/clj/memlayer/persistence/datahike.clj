...
(defn transact-user-settings-schema!
  "Transact the user-settings schema into the database.
   Accepts raw conn (lifecycle, called during init)."
  [conn us-schema]
  (d/transact conn us-schema))
...
(defn get-user-settings
  ...
  (let [conn (:conn store)]
    (first (d/q '[:find [(pull ?e [*]) ...]
                  :in $ ?uid
                  :where [?e :user-settings/user-id ?uid]]
                @conn user-id))))

(defn save-user-settings!
  "Transact a user-settings entity map."
  [store entity]
  (let [conn (:conn store)]
    (d/transact conn [entity])))

(defn find-user-settings-eid
  "Find the entity id for a user-settings record."
  [store user-id]
  (let [conn (:conn store)]
    (d/q '[:find ?e .
           :in $ ?uid
...
(defn retract-user-settings-attrs!
  "Retract specific attributes and update timestamp."
  [store tx-data]
  (let [conn (:conn store)]
