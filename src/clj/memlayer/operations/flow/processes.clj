...
;; :prepare-context
;;
;; Builds context from recent memories and checks the memory limit.
;; If over quota, resolves the promise-chan with an error immediately.
;; ---------------------------------------------------------------------------
   ...
   (fn
     ...
     ([state _in-id msg]
      (let [{:keys [correlation-id items namespace source]} msg
            limit-err (pipeline/check-memory-limit db (:memory-limit tuning))]
        (if limit-err
          (do (corr/resolve! correlation-map correlation-id limit-err)
              [(push-op state
                        {:correlation-id (str correlation-id)
                         ...
                         :source         source
                         :rejected       true
                         :reject-reason  (:error limit-err)})
               {}])
          (let [context-limit (or (:retain-context-limit tuning) 10)
