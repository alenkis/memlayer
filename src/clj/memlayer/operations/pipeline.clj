...
(defn check-memory-limit
  "Check if adding new memories would exceed the configured limit.
   Returns nil when OK, or an error map when over quota."
  [db-conn memory-limit]
  (when memory-limit
    (let [current (dh/count-active-memories db-conn)]
      (when (>= current memory-limit)
