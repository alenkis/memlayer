(ns memlayer.system
  ...
  (:require [integrant.core :as ig]
            ...
            [memlayer.persistence.dynamodb :as dynamodb]
...
(defmethod ig/init-key :persistence/dynamodb [_ {:keys [config]}]
  (let [{:keys [table endpoint] :as ddb-config} (:dynamodb config)]
    (when table
      (log/info "Initializing DynamoDB client")
      (let [client (dynamodb/create-client ddb-config)]
        (when endpoint
          (dynamodb/ensure-table! client table))
        {:client client :table table}))))

(defmethod ig/halt-key! :persistence/dynamodb [_ ddb]
  (when ddb
    (log/info "Stopping DynamoDB client")
    (dynamodb/stop-client (:client ddb))))

(defmethod ig/init-key :persistence/stratum [_ {:keys [db]}]
  (log/info "Initializing stratum analytics (auto-sync from datahike)")
  {:db db :dataset-atom (strat/install-sync-listener! db)})

(defmethod ig/halt-key! :persistence/stratum [_ {:keys [db]}]
  (log/info "Removing stratum sync listener")
...
(defmethod ig/init-key :memlayer/deps [_ {:keys [db vector stratum openai groq config]}]
  (log/info "Building deps")
  {:db                 db
   :vector-index       vector
   :stratum            (some-> stratum :dataset-atom)
...
(def system-config
  {:memlayer/config {}
   ...
   :persistence/dynamodb  {:config (ig/ref :memlayer/config)}
   :persistence/stratum   {:db (ig/ref :persistence/datahike)}
   ...
   :memlayer/deps {:db      (ig/ref :persistence/datahike)
                   ...
                   :stratum (ig/ref :persistence/stratum)
...
(def local-system-config
  "Integrant config for local mode — full functionality, no cloud services.
   No Firebase auth, DynamoDB, or Stratum."
  {:memlayer/config {:path "config-local.edn"}
...
(def mcp-system-config
  "Integrant config for MCP mode — core persistence + LLM providers only.
   No HTTP router, server, DynamoDB, or stratum."
  {:memlayer/config    {}
   :persistence/datahike {:config (ig/ref :memlayer/config)}
   :persistence/proximum {:config (ig/ref :memlayer/config)}
   :provider/openai      {:config (ig/ref :memlayer/config)}
