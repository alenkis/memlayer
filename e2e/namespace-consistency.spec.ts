...
test.describe("Namespace consistency", () => {
  ...
  test("memories are scoped to their namespace across all pages", async ({
    ...
    await test.step(
      "browser shows foo memories including at least one episode",
      async () => {
        await page.goto("/browser");
        // Wait for at least one row to appear before asserting
        await expect(page.locator("tbody tr").first()).toBeVisible({
          timeout: 15_000,
        });
        await expect(page.locator("tbody tr:has-text('episode')")).toBeVisible({
          timeout: 15_000,
        });
      }
    );
