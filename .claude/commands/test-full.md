...
Start both in background. **Both commands need both env vars** (server uses DASHBOARD_PORT for CORS, dashboard uses MEMLAYER_PORT for API requests). The server also needs `AUTH_E2E_MODE=true` to disable Firebase JWT auth — e2e tests use mocked Firebase auth and the admin reset endpoint requires this bypass.

Start the API server:
```bash
AUTH_E2E_MODE=true MEMLAYER_PORT=19001 DASHBOARD_PORT=19002 bb server
```
