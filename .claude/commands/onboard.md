...
If missing, ask the user: **"Do you use 1Password CLI for secrets, or will you add API keys to .env manually?"**

- **1Password**: Run `bb env` to generate `.env` from 1Password references. Verify:
  1. Check `op` is installed: `op --version`
  2. Check authentication: `op whoami`
  3. Check secret access:
     ```bash
     op read "op://MemLayer/OpenAIAPIKey/password"
     op read "op://MemLayer/GroqAPIKey/password"
     ```
- **Manual**: Copy and fill in:
  ```bash
  cp .env.example .env
...
**Project structure:**
- `src/memlayer/` — Clojure backend (operations, persistence, API, MCP)
- `src/cljs/memlayer/dashboard/` — ClojureScript dashboard (Re-frame)
